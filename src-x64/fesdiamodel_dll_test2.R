   ...
   if ( method %in% c("runsteady", "mixed")) {
    DIA <- FESDIAdyna_Full (parms = parms, times = times, gridtype = gridtype, yini = yini, 
     ...
     if (method == "runsteady"){  # finished
       ...
       STD  <- list(y = matrix(nrow = N, ncol = 18, data  = SVAR))
...
# expand forcings to be a "time series"
  ...
  niter <- 1
  y1 <- c(1e2, 1e4, 10, 5, 1, 1000, 10, 1e4,1e4,1e3,1,1e4,1e2,1e4,1e2,0,1e4,1e4)
  y2 <- c(1e1, 1e3, 100, 10, 10, 10, 1, 1e2,1e2,1e3,1e2,1e4,1,3e4,1,0,1e3,1e3)
